...
class cwSaab():
    ...
    def fit(self, X):
        ...
        logger.info(f'layer 0 shape: {X.shape}')
        logger.info('=' * 45 + '>c/w Saab Train Hop 1')
        for i in range(1, self.depth):
            X = self.cwSaab_n_layer(X, train=True, layer=i)
            logger.info(f'layer {i} shape: {X.shape}')
            if (self.split == False and self.SaabArgs[i]['cw'] == True):
                self.depth = i
                logger.info("       <WARNING> Cannot futher split, actual depth: %s" % str(i))
                break
            logger.info('=' * 45 + f'>c/w Saab Train Hop {i + 1}')
            self.split = False
        logger.info('finished Pixelhop training!')
        self.trained = True
